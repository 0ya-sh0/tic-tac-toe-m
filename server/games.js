...
games.prototype.moveHandler = function(socket,gid, tid) {
    console.log(`@move: token: ${tid} @game: ${gid}`);
    this.dict.get(gid).sendMove(tid);
}
...
games.prototype.connectionHandler = function(socket) {
    ...
    socket.on('move',(function(gid, tid){
            this.moveHandler(socket, gid, tid);
        }).bind(this)
    );
    /*
